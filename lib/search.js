...
Search.prototype.setProxy = function (proxyIp) {
    if (/^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5]):[0-9]+$/g.test(proxyIp)) {
    ...
    return new Promise(
      function (resolve, reject) {
          request.post({
              ...
              json: bodyParams,
              proxy: this.proxyIp ? `http://${this.proxyIp}` : null,
              tunnel: false,
              strictSSL: false,
              gzip: true
              ...
              if (jsonResult && jsonResult.startsWith('<!--')) {
                  return reject(`Rejected by Datadome Protection`);
              }

              if (jsonResult.url && jsonResult.url.startsWith('https://c.datado.me')) {
                  return reject(`Rejected by Datadome Protection`);
              }
